...
## output filepath ----------------------------------------------
iv.output.results.file <- "Data/Replication_v2/IV_subsamples_initprice_DLL_DLL.csv"
output.emp.price.dist <- "Data/Replication_v2/Emp_price_subsamples_initprice_DLL_DLL.csv"
  ...
  for (FE in FE_opts) {
    ...
    for (q in unique(all_pi$quantile)) {
      if (nrow(all_pi[quantile == q]) > 0) {
        ...
        ## First-stage 
        formula1 <- as.formula(paste0("DLL.ln_cpricei2 ~ DLL.ln_sales_tax | ", FE, " | 0 | module_by_state"))
        res1 <- felm(formula = formula1, data = all_pi[quantile == q],
                     weights = all_pi[quantile == q]$base.sales)
        
        ## attach results
        res1.dt <- data.table(coef(summary(res1)), keep.rownames=T)
        res1.dt[, outcome := "DLL.ln_cpricei2"]
        res1.dt[, controls := FE]
        res1.dt[, group := q]
        res1.dt[, n.groups := n.g]
        ...
        ## Reduced-Form
        formula1 <- as.formula(paste0("DLL.ln_quantity3 ~ DLL.ln_sales_tax | ", FE, " | 0 | module_by_state"))
        res1 <- felm(formula = formula1, data = all_pi[quantile == q],
                     weights = all_pi[quantile == q]$base.sales)
        
        ## attach results
        res1.dt <- data.table(coef(summary(res1)), keep.rownames=T)
        res1.dt[, outcome := "DLL.ln_quantity3"]
        res1.dt[, controls := FE]
        res1.dt[, group := q]
        res1.dt[, n.groups := n.g]
