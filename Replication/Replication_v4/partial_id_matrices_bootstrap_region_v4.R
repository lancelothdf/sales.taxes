...
theta.bernstein <- "Data/Replication_v4/Demand_gamma_sat_initial_price_semester_boot_r_K_region"
...
for (n.g in 1:3) {
  ...
  # Saturate with FE
  all_pi[, paste0(FE) := .GRP, by = .(region_by_module_by_time, quantile)]
...
for (rep in 1:100) {
  ...
  for (n.g in 1:3) {
    ...
    # Saturate with FE
    sampled.data[, paste0(FE) := .GRP, by = .(region_by_module_by_time, quantile)]
