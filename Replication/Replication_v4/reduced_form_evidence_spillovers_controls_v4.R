...
#' Sales Taxes
#' Replication File. Updated on 3/07/2022
...
## input filepath ----------------------------------------------
#all_pi_spill <- fread("Data/Replication_v2/all_pi_spill.csv")
all_pi_spill_econ <- fread("Data/Replication_v2/all_pi_spill_econ.csv")

## output filepath ----------------------------------------------
results.file.spillovers <- "Data/Replication_v2/LRdiff_semesterly_spillovers_controls.csv"
...
####### Longer-Difference specification ------

## De-mean statutory tax rate
#all_pi_spill[, w.ln_statutory_tax := ln_statutory_tax - mean(ln_statutory_tax, na.rm = T), by = store_by_module]
outcomes <- c("DLL.ln_cpricei2", "DLL.ln_quantity3", "DLL.ln_pricei2", "DLL.ln_sales")
FE_opts <- c("region_by_module_by_time", "division_by_module_by_time")
  ...
  for (sam in subsamples) {
    ...
    for (Y in c(outcomes)) {
      for (FE in FE_opts) {
        ...
        formula1 <- as.formula(paste0(
          Y, "~ DLL.ln_statutory_tax + DL.ln_home_price + DL.ln_unemp | ", FE, " | 0 | module_by_state"
        ))
        flog.info("Estimating with %s as outcome with %s FE in sample %s.", Y, FE, s)
        res1 <- felm(formula = formula1, data = data.est,
                     weights = data.est$base.sales)
