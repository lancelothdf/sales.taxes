...
## output filepath ----------------------------------------------
iv.output.results.file <- "Data/Replication_v4/IV_subsamples_initprice_test.csv"
output.emp.price.dist <- "Data/Replication_v4/Emp_price_subsamples_initprice_test.csv"
...
n.g <- 2
  ...
  for (FE in FE_opts) {
    ...
    # Create a dummy variable for the first quantile
    all_pi[, quantile_1 := as.integer(quantile == 1)]
    #all_pi[, quantile_2 := as.integer(quantile == 2)]  # Optional if testing multiple groups
    
    # Modify the formula to include an interaction
    formula_test <- as.formula(paste0("DL.ln_quantity3 ~ 0 | ", 
                                      FE, 
    ...
    #summary(res_test)
    #linearHypothesis(res_test, "DL.ln_sales_tax:quantile_1 = 0")
