...
# Output
out.welfare.nationwide.av <- "Data/Replication_v4/average_nationwide_extrapolation_division_smallv5.csv"
sol.welfare.nationwide.av <- "Data/Replication_v4/average_nationwide_temp_progress_division_smallv5.csv"
...
## Forget about K=8, L=1, "Original", sigma = 0.5 and theta = 0 for now (will focus on it in a separate file later)
#combinations.all <- data.table(K = rep(8,6), L = c(1, 2, 1, 2, 2, 2), sc = c(rep("Original", 2), rep("plus 5 Tax", 4)), sigma = c(0.75, 0.5, 0.5, 0.25, 0.5, 0.75), theta = rep(0, 6))
combinations.all <- data.table(K = rep(8,2), L = c(2,1), sc = c("No Tax", "Original"), sigma = c(0.25, 0.50), theta = rep(0,2))
