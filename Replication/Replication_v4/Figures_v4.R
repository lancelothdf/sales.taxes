...
data <- fread("Demand_theta_sat_initial_price_semester_salience.csv")
...
#### TEST: check spillovers controls
test <- fread("Data/LRdiff_semesterly_spillovers_controls.csv")
test <- test[subsample == "all_taxexempt" & rn %in% c("DL.ln_statutory_tax", "DLL.ln_statutory_tax") & sample == "non_imp_tax_strong"]
