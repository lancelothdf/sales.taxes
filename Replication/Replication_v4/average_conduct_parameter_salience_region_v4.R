...
# output
conduct.parameter.file <- "Data/Replication_v4/salience_conduct_parameter_at_p_region.csv"
...
rho.old <- rho.old[ outcome != "DL.ln_quantity3" & controls == "region_by_module_by_time" & iter == 0,]

## Betas estimation
betas.old <- fread(theta.output.salience.results.file)
betas.old <- betas.old[controls == "region_by_module_by_time" & iter == 0,]


### Manually incorporate values needed
epsilon <- 0.0000001
ed <- 0.449 # Added manually from full sample estimates with region by module fixed effects
