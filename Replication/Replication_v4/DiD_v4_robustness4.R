...
all_pi <- fread("Data/Replication_v4/all_pi_spill.csv")

## output filepath ----------------------------------------------
output.results.file <- "Data/Replication_v4/LR_Diff_design_robust4.csv"
output.pre.trends.file <- "Data/Replication_v4/LR_Diff_design_pretrends_robust4.csv"
...
for (s in samples) {
  ...
  ## Keep only modules that are always taxable
  #data.est <- all_pi[get(s) == 1,]
  data.est <- all_pi[(all_taxable == 1) & get(s) == 1]
