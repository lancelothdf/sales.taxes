...
## output filepath ----------------------------------------------
iv.output.results.file <- "Data/Replication_v4/IV_invsup_subsamples_initprice.csv"
output.emp.price.dist <- "Data/Replication_v4/Emp_prod_price_subsamples_initprice.csv"
...
all_pi[, n.ln_pricei2 := ln_pricei2 - mean(ln_pricei2, na.rm = T), by = .(module_by_time)]
...
for (n.g in 1:5) {
  ...
  for (FE in FE_opts) {
    ...
    if (pricedist) {
      ...
      step.log.p <- (max(all_pi$ln_pricei2, na.rm = T) - min(all_pi$ln_pricei2, na.rm = T) )/1500
      step.n.log.p <- (max(all_pi$n.ln_pricei2, na.rm = T) - min(all_pi$n.ln_pricei2, na.rm = T)) /1500
      min.log.p <- min(all_pi$ln_pricei2, na.rm = T)
      min.n.log.p <- min(all_pi$n.ln_pricei2, na.rm = T)
      all_pi[, d.lp := floor((ln_pricei2 - min.log.p)/step.log.p)]
      all_pi[, d.n.lp := floor((n.ln_pricei2 - min.n.log.p)/step.n.log.p)]
    ...
    for (q in unique(all_pi$quantile)) {
      if (nrow(all_pi[quantile == q]) > 0) {
        formula1 <- as.formula(paste0("DL.ln_pricei2 ~ 0 | ", 
                                      FE, 
        ...
        ## Reduced-Form
        formula1 <- as.formula(paste0("DL.ln_pricei2 ~ DL.ln_sales_tax | ", FE, " | 0 | module_by_state"))
        res1 <- felm(formula = formula1, data = all_pi[quantile == q],
                     weights = all_pi[quantile == q]$base.sales)
