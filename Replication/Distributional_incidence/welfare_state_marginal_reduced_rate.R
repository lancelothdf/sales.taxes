...
# Output
out.welfare.state.marg <- "Data/Replication/state_welfare_extrapolation_marginal_reduced_rate.csv"
...
data <- data[Food == 1 & taxability == 2,] ## Keep only NonFood items
data[, p_cml := p_m - tau]
...
states.test <- c(5, 17, 29, 37, 47, 49, 51)
...
while (!done) {
  ...
  for (nr in 1:nrow(combinations)) {
    ...
    p.min <- res.pq[extrap == "Original" & sigma == sig & iter == rep][["min.p"]]
    p.max <- res.pq[extrap == "Original" & sigma == sig & iter == rep][["max.p"]]
