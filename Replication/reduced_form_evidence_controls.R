...
for (s in samples) {
  ...
  LRdiff_res$N_obs <- nrow(data.est)
  LRdiff_res$N_modules <- length(unique(data.est$product_module_code))
  LRdiff_res$N_stores <- length(unique(data.est$store_code_uc))
  LRdiff_res$N_counties <- uniqueN(data.est, by = c("fips_state", "fips_county"))
  LRdiff_res$N_years <- uniqueN(data.est, by = c("year")) # should be 6 (we lose one because we difference)
  LRdiff_res$N_county_modules <- uniqueN(data.est, by = c("fips_state", "fips_county",
                                                             "product_module_code"))
  fwrite(LRdiff_res, output.results.file.econ)
}