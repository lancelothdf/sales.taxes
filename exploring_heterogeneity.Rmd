...
As we can see, most of the increases are concentrated in 2010 Q3, and most of the decreases are concentrated in 2011 Q3. What is driving these large spikes?
...
The only state-wide tax change not captured in the previous analysis is Arizona's June 2010 tax increase, which increased state sales tax by 1%, from 5.6% to 6.6%.
...
```
We have some variables obtained from Zillow.com and IPUMS National Historical Geographic Information System (NHGIS) in addition to the Quarterly Census of Employment and Wages (QCEW) and the Local Area Unemployment Statistics, both from the Bureau of Labor Statistics. We can use these data (restricted to information determined prior to the tax change) to see which county characteristics "predict" sales tax changes:
...
`annual_avg_emplvl`[^1]        | QCEW   | Establishment employment level (2006)
...
`employed`[^2]                 | LAUS   | Total employment level (2006)
`unemployed                   `| LAUS   | Unemployed level (2006)
`unemp_rate                   `| LAUS   | Unemployment rate (2006)

[^1]: Note that there are two measures of employment, one from the QCEW (`annual_avg_emplvl`) and one from the LAUS (`employed`). This is because the QCEW estimates employment levels using establishment data, but the BLS adjusts these measures using commutation factors calculated from the Census Bureau’s American Community Survey (ACS). For more information, see the BLS [estimation methodology](https://www.bls.gov/lau/laumthd.htm).

[^2]: See footnote 1.

In addition to these variables, I have industry employment shares for a number of different industries from the QCEW. I adapted many of the vairables in this table to be in percentage or log terms for comparability across counties.

To determine what features are indicative of a future sales tax change, I run predictive regressions similar to those run by Deshpande and Li (2018). I estimate three different models for each type of sales tax reform (increase and decrease). In both cases, I include only counties that underwent the relevant reform (e.g., for the increase outcome, include counties that ever increased their sales tax between 2010 and 2011) as well as counties that never experienced any sales tax reform during the same time period as a control group.

The first model includes population-specific covariates relating to housing, employment, income, race, and education. The second model includes industry employment shares for the county for six specific industries. The third model combines the covariates from the first two models.
<!--
The first model includes population-specific covariates, and is specified as follows:

$$
y_c = \beta_0 + \beta_1\ln(pop2000_c) + \beta_2UnempRate_{c2006} + \beta_3\ln(MedianHomePrice) + 
$$
-->

Because the outcomes are binary (i.e., indicators for ever increase and ever decrease), I estimate the models via logistic regression.
...
res1 <- summary(baseline_model)$coefficients
...
res2 <- summary(model2)$coefficients

full_formula <- as.formula(paste0("ever_increase ~ ",
                                  baseline_covariates, "+",
                                  industry_shares))
model3 <- glm(formula = full_formula, data = tr_groups, family = "binomial")
res3 <- summary(model3)$coefficients

## TODO: think of some good way to present these results

increase_results <- data.table(variable = rownames(res3),
                               coef = res3[, "Estimate"],
                               std_err = res3[, "Std. Error"],
                               z_val = res3[, "z value"],
                               p_val = res3[, "Pr(>|z|)"],
                               model = 3)

# TODO: join on the other two results and put into a pretty table!
...
<!-- TODO: Update data section of this part (once server has finished running)  -->

The goal here is to identify which goods exhibit the most seasonality over time. However, this must be done on the server due to the size of the dataset. On the server, I estimate the following model on the product ($p$)-store ($s$)-quarter ($q$)-year ($y$) level:[^3] $$ y_{psqy} = \alpha_{qp} + \gamma_ptime_{t(q,y)} + \epsilon_{psqy},$$ where $$y_{psqy} = \textrm{ln}\left(\frac{sales_{psqy}}{sales_{psQ_12008}}\right),$$ $\gamma_p$ is a product-specific linear time effect, and $\alpha_{qp}$ is a product-quarter fixed effect. I then calculate a seasonality range for each product, $$SR_p = \max_{q}(\alpha_{qp})-\min_{q}(\alpha_{qp}).$$ $SR_p$ is then essentially the product-specific ratio of normalized log sales of the season with the highest average sales to the season with the lowest average sales.[^4] We use it primarily to compare seasonality across goods.

[^3]: For computational purposes, I instead residualize out the linear time trend to obtain $\tilde{y}_{psqy} = y_{psqy} - \gamma_ptime_{t(q,y)}$ and then take the mean of $\tilde{y}_{psqy}$ over all stores $s$ and years $y$ for each product-quarter pair to obtain $\alpha_{qp}.$ This is functionally equivalent to what the estimated product-quarter fixed effects would be (without an intercept).
...
We can use these resulting $SR_p$ indicators to examine the distribution of seasonality within the 265 best-selling products in our balanced panel.

```{r, echo = FALSE, fig.cap = "Distribution of seasonality range across products"}
...
Most products appear to exhibit some sort of seasonality. However, there are clearly some outliers. Which products are these?

```{r, echo = FALSE}
setorder(seasonality, -seasonality_range)
head(seasonality[, .(module_description, seasonality_range)], 10)
```

Not surprisingly, these are goods one would expect to be correlated with seasons, due to:

* Holidays (chocolate, video games)
* Summer (fans, sunscreens & sunblocks, charcoal, ice)
* Back-to-school (binders & folders)
* Seasonal production (fresh strawberries)
* Seasonal transportation costs (ground beef)
