...
#' This version is to examine **house prices**
...
output.results.filepath <- "Data/homeprice_ed_monthly_regression_res_prodmatch_combined.csv"
output.residuals.normalized.homeprice.filepath <- "Data/homeprice_ed_monthly_regression_prodmatch_residuals_combined.csv"
output.residuals.tax.filepath <-"Data/homeprice_ed_monthly_regression_prodmatch_residuals_tax_combined.csv"
output.xx.filepath <- "Data/homeprice_ed_monthly_regression_prodmatch_xx_combined.csv"
...
all_pi <- merge(zillow_dt, all_pi, by = c("fips_county", "fips_state"))
rm(zillow_dt)
gc()

## normalize
all_pi[, normalized.homeprice := log(median_home_price)]
all_pi <- all_pi[!is.na(normalized.homeprice) & !is.na(base.sales)]
...
clustered.res.normalized.homeprice <- data.table(NULL)
# clustered.res.tax <- data.table(NULL)

#Matrix to store the "sandwiches" for OLS on normalized.homeprice
xx.normalized.homeprice <- data.table(NULL)
...
for (yr in 2009:2013) {
  for (mon in 1:12) {
    ...
    ## estimate for normalized.homeprice =================================================
    felm_formula_input <- paste(new_cols_used, collapse = "+")
    cXp_formula <- as.formula(paste0("normalized.homeprice ~ ", felm_formula_input,
    ...
    setDT(resid)
    resid.normalized.homeprice <- resid[, list(cattlead12 = sum(weights*cattlead12*residuals),
    ...
    resid.normalized.homeprice$ref_year <- yr
    resid.normalized.homeprice$ref_mon <- mon
    resid.normalized.homeprice$n <- dim(ss_pi)[1]

    #Save these sums of "interacted" residuals for each county
    clustered.res.normalized.homeprice <- rbind(clustered.res.normalized.homeprice, resid.normalized.homeprice)
    rm(resid.normalized.homeprice)
    ...
    xx.normalized.homeprice <- rbind(xx.normalized.homeprice, xx.mat)
    ...
    res.cp[, outcome := "normalized.homeprice"]
    setnames(res.cp,
    ...
    fwrite(clustered.res.normalized.homeprice, output.residuals.normalized.homeprice.filepath)
    # fwrite(clustered.res.tax, output.residuals.tax.filepath)
    fwrite(xx.normalized.homeprice, output.xx.filepath)
...
###OUTPUT
output.cov.normalized.homeprice <- "Data/homeprice_ed_monthly_regression_prodmatch_varcov_matrix_combined.csv"
output.skeleton <- "Data/homeprice_ed_monthly_regression_prodmatch_varcov_matrix_skeleton_combined.csv"

### INPUTS
output.results.filepath <- "Data/homeprice_ed_monthly_regression_res_prodmatch_combined.csv"
output.residuals.normalized.homeprice.filepath <- "Data/homeprice_ed_monthly_regression_prodmatch_residuals_combined.csv"
#output.residuals.tax.filepath <-"Data/homeprice_ed_monthly_regression_prodmatch_residuals_tax_combined.csv" ##We do not need this one here because we are interested in unemployment
output.xx.filepath <- "Data/homeprice_ed_monthly_regression_prodmatch_xx_combined.csv"


normalized.homeprice.res <- fread(output.residuals.normalized.homeprice.filepath)
...
list.counties <- unique(normalized.homeprice.res$county_ID)
list.years <- unique(normalized.homeprice.res$ref_year)
list.mon <- unique(normalized.homeprice.res$ref_mon)
...
non.empty.blocks <- normalized.homeprice.res[, .(.N), .(ref_year, ref_mon)] ##Note tax.res[, .(.N), .(product_module_code, ref_year, ref_qtr)] gives exactly the same data.frame
...
#for(cty in list.counties[1:3])
  ...
  ##Price Indices
  c.normalized.homeprice <- normalized.homeprice.res[county_ID == cty]
  c.normalized.homeprice <- c.normalized.homeprice %>% gather(key = "lead", value = "parameter",
  ...
  c.normalized.homeprice <- merge(skeleton, c.normalized.homeprice[,c("ref_year", "ref_mon", "lead", "parameter")], by = c("ref_year", "ref_mon", "lead"), all.x = TRUE)

  setDT(c.normalized.homeprice)
  c.normalized.homeprice <- c.normalized.homeprice[order(ID),] ##Make sure that the sequence follows cattlead4 to cattlead1, catt0 to catt4
  c.normalized.homeprice[is.na(c.normalized.homeprice)] <- 0


## Tried different ways because this step takes a long time mat.mult is slower here
residual.mat <- residual.mat + as.vector(c.normalized.homeprice$parameter)%*%t(as.vector(c.normalized.homeprice$parameter))
...
write.table(residual.mat, "Data/large_vcov_matrices/Mat_residuals_prodmatch_ed_monthly_pi_combined.csv")
...
fwrite(cov.matrix, output.cov.normalized.homeprice)
...
output.estimates.stderr.filepath <- "Data/Homeprice_estimates_stderr_ed_monthly_combined.csv"


cov.matrix <- fread(output.cov.normalized.homeprice)
skeleton <- fread(output.skeleton)
cp.all.res <- fread(output.results.filepath)
K.param <- dim(skeleton)[1]

##Produce standard errors for normalized.homeprice pooled across cohorts
...
cohort.weights <- cp.all.res[outcome == 'normalized.homeprice', "total_sales"]
...
##normalized.homeprice
weights <- setDT(cp.all.res[outcome == 'normalized.homeprice',])
...
est <- mean(estimates[outcome == 'normalized.homeprice' & (rn %in% c('catt0', 'catt1', 'catt2', 'catt3', 'catt4',
                                                        'catt5', 'catt6', 'catt7', 'catt8', 'catt9',
...
temp2 <- as.data.frame(t(as.vector(c("post-treatment", "normalized.homeprice"))))
