#' Author: John Bonney
#'
#' This file creates event-study means plots based on the quantity indices. They
#' are weighted the same as the price indices are weighted (by sales in 2008 Q1).
...
#'
#' E.g., $ Rscript cohort_specific_all_pooled.R "Ever increase" "Data/quantityi_all_cohorts_ei_pooled_taxable.csv"
...
output.filepath <- args[2]
if (!grepl("csv", output.filepath)) {
  stop("`output.filepath` must be a .csv")
}
...
taxable_pi <- taxable_pi[year %in% 2006:2014 & !is.na(cpricei) & !is.na(quantityi)]
...
taxable_pi[, correction := quantityi[year == 2013 & quarter == 1] / quantityi[year == 2012 & quarter == 4],
           by = .(store_code_uc, product_module_code)]
## divide price index after 2013 Q1 (inclusive) by above value
taxable_pi[year >= 2013, quantityi := quantityi / correction]

taxable_pi[, quantityi := log(quantityi)]
...
taxable_pi <- taxable_pi[!is.na(base.sales) & !is.na(quantityi)]
...
control_dt.nt <- control_dt.nt[, list(
  control.quantityi = weighted.mean(x = quantityi, w = base.sales)
...
matched_control_data.nt <- matched_control_data.nt[, .(
  control.quantityi, tt_event, event_ID, store_code_uc, product_module_code,
...
control_dt.ft <- control_dt.ft[, list(
  control.quantityi = weighted.mean(quantityi, w = base.sales)
...
taxable_pi[, normalized.quantityi := quantityi - quantityi[tt_event == -2],
           by = .(store_code_uc, product_module_code, ref_year, ref_quarter,
                  tr_group, event_ID)]
# drops groups for which tt_event == -2 not available
taxable_pi <- taxable_pi[!is.na(normalized.quantityi)]
# note that this is still log quantityi

### this illustrates an issue that we get
# test_collapsed <- taxable_pi[, list(mean.quantityi = weighted.mean(normalized.quantityi, w = base.sales),
...
taxable_pi_es_collapsed <- taxable_pi[,
                                      list(mean_pi = weighted.mean(x = normalized.quantityi, w = base.sales),
