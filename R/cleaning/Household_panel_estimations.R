...
## Filepaths
# Exporting results to my folder
...
purchases.full <- purchases.full[, share_expend := total_expenditures/sum_total_exp ]
...
desc.est  <- data.table(describe(purchases.full[, .(ln_sales_tax, ln_cpricei, ln_quantity, ln_share_expend,
                                         total_expenditures, share_expend, ln_share_expend_100)]), keep.rownames=T)
