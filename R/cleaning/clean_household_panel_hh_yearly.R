...
## Calculate total expenditure per consumer in each (across stores and modules)
...
purchases.full <- purchases.full[, list(
            total_expenditures = sum(total_expenditures)
                    ...
                    same_3zip_store, year, sum_total_exp, projection_factor,
...
purchases.full <- dcast(purchases.full, household_code + taxability + fips_county_code + fips_state_code +
                          zip_code + year + projection_factor + projection_factor_magnet +
                          sum_total_exp + household_income + region_code ~ same_3zip_store, fun=sum,
...
purchases.full <- dcast(purchases.full, household_code + fips_county_code + fips_state_code + zip_code
                        + year + projection_factor + projection_factor_magnet + sum_total_exp + region_code +
...
fwrite(purchases.full, "cleaning/consumer_panel_y_hh_2006-2016.csv")