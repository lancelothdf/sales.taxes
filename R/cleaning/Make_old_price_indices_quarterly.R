...
for(i in 1:length(list.modules)) {
  ...
  index.input <- paste(output.index_dir, "Index_group_", group, "_module_", module, ".csv", sep = "")

  mod.pi <- fread(index.input)
  mod.pi <- mod.pi[, c("store_code_uc", "product_module_code", "year", "quarter", "pindex", "constant")]


  all_pi <- rbind(all_pi, mod.pi)
}

names(all_pi) <- c("store_code_uc", "product_module_code", "year", "quarter", "ln_pricei2", "constant")
