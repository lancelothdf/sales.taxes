...
## quantity plots ---------------------
## setup -------------------
res.quantity <- fread("pi_data/semesterly_quantity_output_pooled.csv")
res.quantity <- res.quantity[rn %in% paste0(c("L0", "L1", "L2", "L3", "L4", "F1", "F3", "F4", "Pre", "Post", "All"), ".ln_sales_tax")]
# res.quantity <- res.quantity[controls %in% c("module_by_time", "region_by_module_by_time")]

## back out the lead/lag
res.quantity$tt_event <- as.double(stringr::str_match(res.quantity$rn, "[0-9]"))
res.quantity[is.na(tt_event), tt_event := ifelse(grepl("Pre", rn), -6,
                                            ifelse(grepl("Post", rn), 6,
                                                   ifelse(grepl("All", rn), 8.5, 0)))]
res.quantity[, tt_event := ifelse(grepl("F", rn), -1 * tt_event, tt_event)]
res.quantity[, aggregated := tt_event %in% c(-6, 6, 8.5)]

setnames(res.quantity, old = c("Estimate", "Cluster s.e."), new = c("estimate", "se"))

## module-by-time FE
res.quantity <- res.quantity[outcome == "ln_quantity" & controls == "module_by_time"]
res.quantity <- rbind(res.quantity, data.table(tt_event = -2, estimate = 0, aggregated = F), fill = T)
ggplot(data = res.quantity, mapping = aes(x = tt_event, y = estimate, color = aggregated)) +
  geom_point(size = 2, alpha = .5) +
  geom_errorbar(data = res.quantity[tt_event != -2],
                aes(ymax = estimate + 1.96 * se,
                    ymin = estimate - 1.96 * se),
                width = .6) +
  ...
  theme(legend.position = "none",
        panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank())
ggsave("pi_figs/reg_results/pooled/quantity_module_by_time.png",
       height = 120, width = 180, units = "mm")
