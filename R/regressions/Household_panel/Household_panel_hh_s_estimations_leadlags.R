...
## Open Data
purchases.full <- fread("cleaning/consumer_panel_s_hh_2006-2016.csv")

purchases.full$time <- factor(with(purchases.full, interaction(year, semester)))
...
## Shares
# type or taxability
purchases.sample[, share_taxable := expenditure_taxable/sum_total_exp_semester]
purchases.sample[, share_non_taxable := expenditure_non_taxable/sum_total_exp_semester]
purchases.sample[, share_unknown := expenditure_unknown/sum_total_exp_semester]
purchases.sample[, share_same3 := expenditure_same3/sum_total_exp_semester]
purchases.sample[, share_diff3 := expenditure_diff3/sum_total_exp_semester]

# type x taxability
purchases.sample[, share_taxable_same3 := expenditures_same3_1/sum_total_exp_semester]
purchases.sample[, share_taxable_diff3 := expenditures_diff3_1/sum_total_exp_semester]
purchases.sample[, share_non_taxable_same3 := expenditures_same3_0/sum_total_exp_semester]
purchases.sample[, share_non_taxable_diff3 := expenditures_diff3_0/sum_total_exp_semester]
purchases.sample[, share_unknown_same3 := expenditures_same3_2/sum_total_exp_semester]
purchases.sample[, share_unknown_diff3 := expenditures_diff3_2/sum_total_exp_semester]
...
# Time
purchases.sample[, cal_time := 2 * year + semester]

# time x region
purchases.sample[, region_by_time := .GRP, by = .(region_code, year, semester)]

# impute tax rates prior to 2008 and after 2014
purchases.sample[, ln_sales_tax := ifelse(year < 2008, ln_sales_tax[year == 2008 & semester == 1], ln_sales_tax),
       by = .(household_code)]
purchases.sample[, ln_sales_tax := ifelse(year > 2014, ln_sales_tax[year == 2014 & semester == 2], ln_sales_tax),
       by = .(household_code)]

## take first differences of outcomes and treatment
setkey(purchases.sample, household_code, year, semester)
purchases.sample <- purchases.sample[order(household_code, cal_time),] ##Sort on hh by year-semester (in ascending order)
...
purchases.sample <- purchases.sample[ year >= 2009 | (year == 2008 & semester >= 2)] ## First semester of 2008, the difference was imputed not real data - so we drop it


## Estimation Set up --------
output.decriptives.file <- "../../../../../home/slacouture/HMS/HH_semester_leadslags_describe.csv"
output.results.file <- "../../../../../home/slacouture/HMS/HH_semester_leadslags_cumulative.csv"
...
for (FE in FE_opts) {
  for (Y in c(outcomes, outcomes_t)) {
    ...
    ## linear hypothesis results
    lp.dt <- data.table(
      rn = c("cumul.lead4.D.ln_sales_tax", "cumul.lead3.D.ln_sales_tax", "cumul.lead2.D.ln_sales_tax", "cumul.lead1.D.ln_sales_tax", 
             "cumul.lag0.D.ln_sales_tax", "cumul.lag1.D.ln_sales_tax", "cumul.lag2.D.ln_sales_tax", "cumul.lag3.D.ln_sales_tax", "cumul.lag4.D.ln_sales_tax"),
      Estimate = c(cumul.lead4.est, cumul.lead3.est, cumul.lead2.est, cumul.lead1.est, 
                   cumul.lag0.est, cumul.lag1.est, cumul.lag2.est, cumul.lag3.est, cumul.lag4.est),
      `Cluster s.e.` = c(cumul.lead4.se, cumul.lead3.se, cumul.lead2.se, cumul.lead1.se, 
                       cumul.lag0.se, cumul.lag1.se, cumul.lag2.se, cumul.lag3.se, cumul.lag4.se),
      `Pr(>|t|)` = c(cumul.lead4.pval, cumul.lead3.pval, cumul.lead2.pval, cumul.lead1.pval, 
                     cumul.lag0.pval, cumul.lag1.pval, cumul.lag2.pval, cumul.lag3.pval, cumul.lag4.pval),
      outcome = Y,
      spec = FE,
      Rsq = summary(res1)$r.squared,
      adj.Rsq = summary(res1)$adj.r.squared)
