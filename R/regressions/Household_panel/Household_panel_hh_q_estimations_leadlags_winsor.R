...
## Estimation Set up --------
output.decriptives.file <- "../../../../../home/slacouture/HMS/HH_quarter_leadslags_describe_winsor.csv"
output.results.file <- "../../../../../home/slacouture/HMS/HH_quarter_leadslags_cumulative_winsor.csv"
...
## Winsorize outcomes (2%) -------
purchases.sample <- purchases.sample[ , c(outcomes, outcomes_t) := (lapply(.SD, Winsorize, probs = c(0.02,0.98), na.rm = T)), .SDcols = c(outcomes, outcomes_t)]
...
for (Y in c(outcomes, outcomes_t)) {
  ...
  lp.dt <- data.table(
    ...
    `Std. Error` = c(cumul.lead8.se, cumul.lead7.se, cumul.lead6.se, cumul.lead5.se, cumul.lead4.se, cumul.lead3.se, cumul.lead2.se, cumul.lead1.se, 
