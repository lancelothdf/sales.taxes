...
# purchases (for which taxability is identified). In this version we split estimations on heterogeneity groups
...
st.taxable.expenditure <- st.taxable.expenditure[ year < 2008 , list(state_sh_expenditure_taxable = sum(expenditure_taxable, na.rm = T)/sum(sum_total_exp_quarter, na.rm = T)), 
                                                  by = .(fips_state_code)]
purchases.sample <- merge(purchases.sample, st.taxable.expenditure, by = "fips_state_code", all.x =T)
# Create taxable consumer: compute its average share of consumption on taxable in "pre" period
hh.taxable.expenditure <- purchases.sample[, list(expenditure_taxable = sum(expenditure_taxable, na.rm = T),
                                                  sum_total_exp_quarter = mean(sum_total_exp_quarter, na.rm = T)), 
                                           by = .(household_code, quarter, year)]
hh.taxable.expenditure <- hh.taxable.expenditure[ year < 2008, list(share_taxable = sum(expenditure_taxable, na.rm = T)/sum(sum_total_exp_quarter, na.rm = T))
                                                  , by = .(household_code)]
...
## Estimation Set up --------
output.decriptives.file <- "../../../../../home/slacouture/HMS/HH_group_quarter_leadslags_describe_taxbase.csv"
output.results.file <- "../../../../../home/slacouture/HMS/HH_group_quarter_distributed_lags_taxbase.csv"
...
for (group in above_or_below) {
  descriptives <- describe(purchases.sample[large_tax_base == group,
                                            .(D.ln_expenditures, D.ln_expenditures_taxable, 
                                              D.ln_expenditures_non_taxable, D.ln_sales_tax, D.ln_statutory_sales_tax,
...
## Run Estimations on effective rate ------
  ...
  for (FE in FE_opts) {
    for (Y in outcomes) {
      ...
      lp.dt <- data.table(
        ...
        N_hholds = length(unique(estimation.sample$household_code))
      )
...
output.results.file <- "../../../../../home/slacouture/HMS/HH_group_quarter_distributed_lags_statutory_taxbase.csv"
...
## Run Estimations on statutory rate ------
...
output.decriptives.file <- "../../../../../home/slacouture/HMS/HH_group_quarter_leadslags_describe_taxableconsumer.csv"
output.results.file <- "../../../../../home/slacouture/HMS/HH_group_quarter_distributed_lags_taxableconsumer.csv"
...
## Run Estimations on effective tax rate ------
...
## Estimation Set up on statutory --------
output.results.file <- "../../../../../home/slacouture/HMS/HH_group_quarter_distributed_lags_statutory_taxableconsumer.csv"
