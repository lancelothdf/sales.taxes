...
# purchases (for which taxability is identified). In this code we restrict the estimation sample to those 
# households that that had initially shopped at a store located in a different 3-digit zip code area
...
# Identify households that had initially (before 2008) shopped at store located in a different 3-digit zip
# It is worth noting that for most stores that code is unidentifyed
pre2008.diff3.purchases <- purchases.sample[year < 2008, list(diff3.shopper = (sum(expenditures_diff3) > 0))
                                            , by = .(household_code)]
purchases.sample <- merge(purchases.sample, pre2008.diff3.purchases, by = "household_code")
# Keep only interest households
purchases.sample <- purchases.sample[diff3.shopper == T, ]
...
## Estimation Set up --------
output.decriptives.file <- "../../../../../home/slacouture/HMS/HH_group_quarter_leadslags_describe_diff3shoppers.csv"
output.results.file <- "../../../../../home/slacouture/HMS/HH_group_quarter_distributed_lags_diff3shoppers.csv"
