...
###OUTPUT
output.results.file <- "Data/Demand_estimation_3years_diff.csv"
...
yearly_data[, cal_time := (year-2008)/6] ## Of course normalization here does not matter to estimation - normalization chosen here so that with two year intervals, cal_time will take values 
...
## !!! Only include 2011 - 2014 to include 2008-2011 and 2011-2014 differences
## ---------------------------------------------------
yearly_data <- yearly_data[year >= 2011 & year <= 2014,]
...
Econ_opts <- c("D.ln_unemp + lag_D.ln_unemp + D.ln_home_price + lag_D.ln_home_price")
...
for(FE in FE_opts) {
  ...
  formula1 <- as.formula(paste0(
    "D.ln_quantity ~ D.ln_cpricei | ", FE, " | 0 | state_by_module "
  ))
  ...
  formula1 <- as.formula(paste0(
    "D.ln_quantity2 ~ D.ln_cpricei2 | ", FE, " | 0 | state_by_module "
  ))
  ...
  flog.info("Writing results...")
  res1.dt <- data.table(coef(summary(res1)), keep.rownames=T)
  res1.dt[, outcome := "D.ln_quantity2"]
  res1.dt[, controls := FE]
  res1.dt[, econ := "none"]
  res1.dt[, lag.econ := NA]
  res1.dt[, instrument := "OLS"]
  ...
  ## Formula - ln_quantity and ln_cpricei
  formula1 <- as.formula(paste0(
    "D.ln_quantity ~ 0 | ", FE, " | (D.ln_cpricei ~ D.ln_sales_tax) | state_by_module "
  ))
  ...
  ## Formula - ln_quantity2 and ln_cpricei2
  formula1 <- as.formula(paste0(
    "D.ln_quantity2 ~ 0 | ", FE, " | (D.ln_cpricei2 ~ D.ln_sales_tax) | state_by_module "
  ))
...
for(FE in FE_opts) {
  for(EC in Econ_opts) {
    ...
    ## Formula - ln_quantity and ln_cpricei
    formula1 <- as.formula(paste0(
      "D.ln_quantity ~ D.ln_cpricei + ", EC, " | ", FE, " | 0 | state_by_module "
    ))
    ...
    res1.dt[, outcome := "D.ln_quantity"]
    res1.dt[, controls := FE]
    res1.dt[, econ := "D.ln_unemp + D.ln_home_price"]
    ...
    ## Formula - ln_quantity2 and ln_cpricei2
    formula1 <- as.formula(paste0(
      "D.ln_quantity2 ~ D.ln_cpricei2 + ", EC, " | ", FE, " | 0 | state_by_module "
    ))
    ...
    ## Formula - ln_quantity and ln_cpricei
    formula1 <- as.formula(paste0(
      "D.ln_quantity ~ ", EC, " | ", FE, " | (D.ln_cpricei ~ D.ln_sales_tax) | state_by_module "
    ))
    ...
    ## Formula - ln_quantity2 and ln_cpricei2
    formula1 <- as.formula(paste0(
      "D.ln_quantity2 ~ ", EC, " | ", FE, " | (D.ln_cpricei2 ~ D.ln_sales_tax) | state_by_module "
    ))
...
## Formula - ln_quantity and ln_cpricei
formula1 <- as.formula(paste0(
  "D.ln_quantity ~ D.ln_cpricei | county_by_time | 0 | state_by_module "
))
...
## Formula - ln_quantity2 and ln_cpricei2
formula1 <- as.formula(paste0(
  "D.ln_quantity2 ~ D.ln_cpricei2 | county_by_time | 0 | state_by_module "
))
...
## Formula - ln_quantity and ln_cpricei
formula1 <- as.formula(paste0(
  "D.ln_quantity ~ D.ln_cpricei | county_by_module_by_time | 0 | state_by_module "
))
...
## Formula - ln_quantity2 and ln_cpricei2
formula1 <- as.formula(paste0(
  "D.ln_quantity2 ~ D.ln_cpricei2 | county_by_module_by_time | 0 | state_by_module "
))
