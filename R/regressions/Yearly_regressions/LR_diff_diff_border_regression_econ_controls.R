...
for(j in 1:length(list.outcomes)) {
  ...
  LRdiff_res <- data.table(
    ...
    ## with module-time FE and Econ controls
    price_formula <- as.formula(paste0(
      outcome.j.ms, " ~ ln_sales_tax_ms + ln_unemp_ms + ln_home_price_ms | pair_by_module_by_time | 0 | state_by_module "
    ))
    ...
    price_res <- felm(data = yearly_data,
      ...
      LRdiff_res <-rbind(
        ...
        data.table(
          ...
          se = coef(summary(price_res))["ln_sales_tax_ms", "Cluster s.e."],
          pval = coef(summary(price_res))["ln_sales_tax_ms", "Pr(>|t|)"],
          est_house = coef(summary(price_res4))["ln_home_price_ms", "Estimate"],
          se_house = coef(summary(price_res4))["ln_home_price_ms", "Cluster s.e."],
          pval_house = coef(summary(price_res4))["ln_home_price_ms", "Pr(>|t|)"],
          est_unemp = coef(summary(price_res4))["ln_unemp_ms", "Estimate"],
          se_unemp = coef(summary(price_res4))["ln_unemp_ms", "Cluster s.e."],
          pval_unemp = coef(summary(price_res4))["ln_unemp_ms", "Pr(>|t|)"],
          Rsq = summary(price_res)$r.squared,
          adj.Rsq = summary(price_res)$adj.r.squared,
          time_controls = "pair by module by time FE",
  ...
  print(paste("Done - ", outcome.j, "mod X pair X year FE - with Econ controls"))
}
...
fwrite(LRdiff_res, "Data/LRdiff_results_county_pairs_econ_controls.csv")
