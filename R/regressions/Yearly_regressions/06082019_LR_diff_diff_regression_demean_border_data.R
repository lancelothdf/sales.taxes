...
print("Saved the mean by store-module")
##

## pair-module-year mean
yearly_data[, mean_ltax_pmy := weighted.mean(ln_sales_tax, w = weight), by = .(bordindx, year, product_module_code)]
print("Calculated store-module mean ln_sales_tax")

yearly_data[, mean_lcpricei_pmy := weighted.mean(ln_cpricei, w = weight), by = .(bordindx, year, product_module_code)]
print("Calculated store-module mean ln_cpricei")

yearly_data[, mean_lcpricei2_pmy := weighted.mean(ln_cpricei2, w = weight), by = .(bordindx, year, product_module_code)]
print("Calculated store-module mean ln_cpricei2")

yearly_data[, mean_lquantity_pmy := weighted.mean(ln_quantity, w = weight), by = .(bordindx, year, product_module_code)]
print("Calculated store-module mean ln_quantity")

yearly_data[, mean_lquantity2_pmy := weighted.mean(ln_quantity2, w = weight), by = .(bordindx, year, product_module_code)]
print("Calculated store-module mean ln_quantity2")

yearly_data[, mean_lunemp_pmy := weighted.mean(ln_unemp, w = weight), by = .(bordindx, year, product_module_code)]
print("Calculated store-module mean ln_unemp")

yearly_data[, mean_lhprice_pmy := weighted.mean(ln_home_price, w = weight), by = .(bordindx, year, product_module_code)]
print("Calculated store-module mean ln_home_price")
...
## Border-year mean
yearly_data[, mean_ltax_py := weighted.mean(ln_sales_tax, w = weight), by = .(bordindx, year)]
print("Calculated store-module mean ln_sales_tax")

yearly_data[, mean_lcpricei_py := weighted.mean(ln_cpricei, w = weight), by = .(bordindx, year)]
print("Calculated store-module mean ln_cpricei")

yearly_data[, mean_lcpricei2_py := weighted.mean(ln_cpricei2, w = weight), by = .(bordindx, year)]
print("Calculated store-module mean ln_cpricei2")

yearly_data[, mean_lquantity_py := weighted.mean(ln_quantity, w = weight), by = .(bordindx, year)]
print("Calculated store-module mean ln_quantity")

yearly_data[, mean_lquantity2_py := weighted.mean(ln_quantity2, w = weight), by = .(bordindx, year)]
print("Calculated store-module mean ln_quantity2")

yearly_data[, mean_lunemp_py := weighted.mean(ln_unemp, w = weight), by = .(bordindx, year)]
print("Calculated store-module mean ln_unemp")

yearly_data[, mean_lhprice_py := weighted.mean(ln_home_price, w = weight), by = .(bordindx, year)]
print("Calculated store-module mean ln_home_price")

##
fwrite(yearly_data, "Data/Nielsen/demeaned_yearly_data.csv")
print("Saved the mean by border-year")
##
...
## First create variable for regression with pair X module X year FE
temp.mean <- weighted.mean(yearly_data$ln_sales_tax, w = yearly_data$weight)
yearly_data[, ln_sales_tax_pmy := ln_sales_tax - mean_ltax_ms - mean_ltax_pmy + temp.mean]

temp.mean <- weighted.mean(yearly_data$ln_cpricei, w = yearly_data$weight)
yearly_data[, ln_cpricei_pmy := ln_cpricei - mean_lcpricei_ms - mean_lcpricei_pmy + temp.mean]

temp.mean <- weighted.mean(yearly_data$ln_cpricei2, w = yearly_data$weight)
yearly_data[, ln_cpricei2_pmy := ln_cpricei2 - mean_lcpricei2_ms - mean_lcpricei2_pmy + temp.mean]

temp.mean <- weighted.mean(yearly_data$ln_quantity, w = yearly_data$weight)
yearly_data[, ln_quantity_pmy := ln_quantity - mean_lquantity_ms - mean_lquantity_pmy + temp.mean]

temp.mean <- weighted.mean(yearly_data$ln_quantity2, w = yearly_data$weight)
yearly_data[, ln_quantity2_pmy := ln_quantity2 - mean_lquantity2_ms - mean_lquantity2_pmy + temp.mean]

temp.mean <- weighted.mean(yearly_data$ln_unemp, w = yearly_data$weight)
yearly_data[, ln_unemp_pmy := ln_unemp - mean_lunemp_ms - mean_lunemp_pmy + temp.mean]

temp.mean <- weighted.mean(yearly_data$ln_home_price, w = yearly_data$weight)
yearly_data[, ln_home_price_pmy := ln_home_price - mean_lhprice_ms - mean_lhprice_pmy + temp.mean]

## Second create variables for regressions with pair X year FE
temp.mean <- weighted.mean(yearly_data$ln_sales_tax, w = yearly_data$weight)
yearly_data[, ln_sales_tax_py := ln_sales_tax - mean_ltax_ms - mean_ltax_py + temp.mean]

temp.mean <- weighted.mean(yearly_data$ln_cpricei, w = yearly_data$weight)
yearly_data[, ln_cpricei_py := ln_cpricei - mean_lcpricei_ms - mean_lcpricei_py + temp.mean]

temp.mean <- weighted.mean(yearly_data$ln_cpricei2, w = yearly_data$weight)
yearly_data[, ln_cpricei2_py := ln_cpricei2 - mean_lcpricei2_ms - mean_lcpricei2_py + temp.mean]

temp.mean <- weighted.mean(yearly_data$ln_quantity, w = yearly_data$weight)
yearly_data[, ln_quantity_py := ln_quantity - mean_lquantity_ms - mean_lquantity_py + temp.mean]

temp.mean <- weighted.mean(yearly_data$ln_quantity2, w = yearly_data$weight)
yearly_data[, ln_quantity2_py := ln_quantity2 - mean_lquantity2_ms - mean_lquantity2_py + temp.mean]

temp.mean <- weighted.mean(yearly_data$ln_unemp, w = yearly_data$weight)
yearly_data[, ln_unemp_py := ln_unemp - mean_lunemp_ms - mean_lunemp_py + temp.mean]

temp.mean <- weighted.mean(yearly_data$ln_home_price, w = yearly_data$weight)
yearly_data[, ln_home_price_py := ln_home_price - mean_lhprice_ms - mean_lhprice_py + temp.mean]
...
for(j in 1:length(list.outcomes)) {
  ...
  outcome.j.pmy <- paste(outcome.j, "_pmy", sep = "")
  outcome.j.py <- paste(outcome.j, "_py", sep = "")

  ## no FE (only store_module and time)
  price_formula1 <- as.formula(paste0(
    outcome.j.py, " ~ ln_sales_tax_py | module_by_time | 0 | state_by_module "
  ))
  ...
  if(j == 1) {
    LRdiff_res <-
      data.table(
        outcome = outcome.j,
        estimate = coef(summary(price_res1))["ln_sales_tax_py", "Estimate"],
        se = coef(summary(price_res1))["ln_sales_tax_py", "Cluster s.e."],
        pval = coef(summary(price_res1))["ln_sales_tax_py", "Pr(>|t|)"],
  ...
  fwrite(LRdiff_res, "Data/LRdiff_results_county_pairs2.csv")


  ## with module-time FE
  price_formula <- as.formula(paste0(
    outcome.j.pmy, " ~ ln_sales_tax_pmy | 0 | 0 | state_by_module "
  ))
  ...
  LRdiff_res <-rbind(
    ...
    data.table(
      outcome = outcome.j,
      estimate = coef(summary(price_res))["ln_sales_tax_pmy", "Estimate"],
      se = coef(summary(price_res))["ln_sales_tax_pmy", "Cluster s.e."],
      pval = coef(summary(price_res))["ln_sales_tax_pmy", "Pr(>|t|)"],
