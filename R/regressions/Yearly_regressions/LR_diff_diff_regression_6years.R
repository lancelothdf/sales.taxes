...
###OUTPUT
output.results.file <- "Data/LRdiff_6years_FE.csv"
...
yearly_data[, cal_time := (year-2008)/6] ## Of course normalization here does not matter to estimation - normalization chosen here so that with two year intervals, cal_time will take values 0, and 1
...
yearly_data <- yearly_data[year %in% c(2008, 2014),]
yearly_data <- yearly_data[order(store_code_uc, product_module_code, year),]



## !!! Only include 2008 - 2014 to create one long difference
## ---------------------------------------------------
