...
yearly_data[, ln_sales_tax_pmy := ln_sales_tax - mean_ltax_pmy]
yearly_data[, ln_cpricei_pmy := ln_cpricei - mean_lcpricei_pmy]
yearly_data[, ln_cpricei2_pmy := ln_cpricei2 - mean_lcpricei2_pmy]
yearly_data[, ln_quantity_pmy := ln_quantity - mean_lquantity_pmy]
yearly_data[, ln_quantity2_pmy := ln_quantity2 - mean_lquantity2_pmy]
yearly_data[, ln_unemp_pmy := ln_unemp - mean_lunemp_pmy]
yearly_data[, ln_home_price_pmy := ln_home_price - mean_lhprice_pmy]

## Second create variables for regressions with pair X year FE
yearly_data[, ln_sales_tax_py := ln_sales_tax - mean_ltax_py]
yearly_data[, ln_cpricei_py := ln_cpricei - mean_lcpricei_py]
yearly_data[, ln_cpricei2_py := ln_cpricei2 - mean_lcpricei2_py]
yearly_data[, ln_quantity_py := ln_quantity - mean_lquantity_py]
yearly_data[, ln_quantity2_py := ln_quantity2 - mean_lquantity2_py]
yearly_data[, ln_unemp_py := ln_unemp - mean_lunemp_py]
yearly_data[, ln_home_price_py := ln_home_price - mean_lhprice_py]
...
for(j in 1:length(list.outcomes)) {
  ...
  price_formula1 <- as.formula(paste0(
    outcome.j.py, " ~ ln_sales_tax_py | store_module + module_by_time | 0 | state_by_module "
  ))
  ...
  price_formula <- as.formula(paste0(
    outcome.j.pmy, " ~ ln_sales_tax_pmy | store_module | 0 | state_by_module "
  ))
