...
for(j in 1:length(list.outcomes)) {
  ...
  price_formula1 <- as.formula(paste0(
    outcome.j, " ~ ln_sales_tax | store_module + module_by_time + pair_by_time | 0 | state_by_module "
  ))
  ...
  price_formula <- as.formula(paste0(
    outcome.j, " ~ ln_sales_tax | store_module + pair_by_module_by_time | 0 | state_by_module "
  ))
