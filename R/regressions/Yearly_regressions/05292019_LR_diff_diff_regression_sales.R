...
### First add years 2006, 2007, 2015 and 2016
# (We need to impute the sales tax rate for those years)


### First produce graphs to explore pre-trends

  ## Start with log price index (1)
  diff_data <- yearly_data[, c("year", "fips_state", "fips_county", "store_code_uc", "product_module_code", "dltax", "ln_cpricei")]


  # Compute residualized price index for different designs
  reg_formula <- as.formula(paste0(
    "ln_cpricei ~ 0 | yr | 0 | state_by_module "
  ))
  ...
  reg_formula <- as.formula(paste0(
    "ln_cpricei ~ 0 | module_by_time | 0 | state_by_module "
  ))
  ...
  reg_formula <- as.formula(paste0(
    "ln_cpricei ~ 0 | store_by_time | 0 | state_by_module "
  ))
  ...
  reg_formula <- as.formula(paste0(
    "ln_cpricei ~ 0 | module_by_time + store_by_time | 0 | state_by_module "
  ))
  ...
  for (k in 2009:2014) {
    ...
    increase_store_modules <- data.year.k[dltax > 0, c("store_code_uc", "product_module_code")]
    nochange_store_modules <- data.year.k[dltax == 0, c("store_code_uc", "product_module_code")]
    decrease_store_modules <- data.year.k[dltax < 0, c("store_code_uc", "product_module_code")]
    ...
    for(j in (k-3):(k+3)) {
      ...
      if( j <= 2016) {
        ...
        #data.year.j <- paste0("data_", j, sep = "")
        #assign(data_year, diff_data[year == k,])
        #test <- weighted.mean(get(data_year)$dltax, w = get(data_year)$base.sales)

        data.year.j <- diff_data[year == j, ]
...
#test <- yearly_data[, list( = weighted.mean(normalized.cpricei, w = base.sales)),
#                              by = c("fips_state", "tt_event")]
#data_2009 <- yearly_data[year == 2009,]
#perc <- quantile(data_2009$dltax, w = data_2009$base.sales, probs = c(0.33, 0.5, 0.66), na.rm = TRUE)
