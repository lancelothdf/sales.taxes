...
#' In this case, we run a semi saturated model 
#' We run only the long run model
...
## output filepaths ----------------------------------------------
output.results.file <- "Data/DID_semisat_initial_price_semester.csv"
...
for (Y in c(outcomes.within)) {
  for (n.g in 2:8) {
    for (n.sat in 1:n.g) {
      ...
      all_pi[, group_division_by_module_by_time := .GRP, by = .(division_by_module_by_time, quantile_sat)]
      
      for (FE in FE_opts) {
        
        formula1 <- as.formula(paste0(
          Y, "~ w.ln_sales_tax:quantile | ", FE, "+ quantile_sat | 0 | module_by_state"
        ))
