  ...
  for(co in c_ids) {
    for (Y in c(outcomes)) {
      for (FE in FE_opts) {
        ...
        res1.dt[, N_modules := length(unique(sampled.data$product_module_code))]
        res1.dt[, N_stores :=  length(unique(sampled.data$store_code_uc))]
        res1.dt[, N_counties := uniqueN(sampled.data, by = c("fips_state", "fips_county"))]
