...
## output filepaths ----------------------------------------------
output.results.file <- "Data/LRdiff_semesterly_w_econ.csv"
...
## Create list of all lagged econ variables
...
lag.econ <- paste(lag.home, " + D.ln_home_price + ", lag.unemp, " + D.ln_unemp + ", lag.wage, " + D.ln_wage")
econ_controls <- c("D.ln_home_price + D.ln_unemp + D.ln_wage", lag.econ, "D.ln_unemp + lag.unemp0 + lag.unemp1 + D.ln_home_price + lag.home_price0 + lag.home_price1 + D.ln_wage + lag.wage0 + lag.wage1")
econ.controls.names <- c("current only", "lagged and current", "current and polynomial in lags")
