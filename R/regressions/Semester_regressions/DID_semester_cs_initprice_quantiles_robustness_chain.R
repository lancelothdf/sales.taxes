...
for (n.g in 1:7) {
  ...
  # Create groups of initial values of tax rate
  # We use the full weighted distribution
  groceries <- groceries[, quantile := cut(dm.L.ln_cpricei2,
                                     breaks = quantile(dm.L.ln_cpricei2, probs = seq(0, 1, by = 1/n.g), na.rm = T, weight = base.sales),
                                     labels = 1:n.g, right = FALSE)]
  quantlab <- round(quantile(groceries$dm.L.ln_cpricei2, 
                             probs = seq(0, 1, by = 1/n.g), na.rm = T, 
                             weight = groceries$base.sales), digits = 4)
  # Saturate fixed effects
  groceries[, group_region_by_module_by_time := .GRP, by = .(region_by_module_by_time, quantile)]
  groceries[, group_division_by_module_by_time := .GRP, by = .(division_by_module_by_time, quantile)]
  ...
  for (FE in FE_opts) {
    for (Y in outcomes) {
      ...
      ##### Only Groceries
  nonDGsample <- all_pi[is.na(DGsample)]
  
  # Create groups of initial values of tax rate
  # We use the full weighted distribution
  nonDGsample <- nonDGsample[, quantile := cut(dm.L.ln_cpricei2,
                                           breaks = quantile(dm.L.ln_cpricei2, probs = seq(0, 1, by = 1/n.g), na.rm = T, weight = base.sales),
                                           labels = 1:n.g, right = FALSE)]
  quantlab <- round(quantile(nonDGsample$dm.L.ln_cpricei2, 
                             probs = seq(0, 1, by = 1/n.g), na.rm = T, 
                             weight = nonDGsample$base.sales), digits = 4)
  # Saturate fixed effects
  nonDGsample[, group_region_by_module_by_time := .GRP, by = .(region_by_module_by_time, quantile)]
  nonDGsample[, group_division_by_module_by_time := .GRP, by = .(division_by_module_by_time, quantile)]
  ...
  for (FE in FE_opts) {
    for (Y in outcomes) {
      ...
      if (n.g == 1) { formula1 <- as.formula(paste0(Y, " ~ w.ln_sales_tax | ", FE)) }
      res1 <- felm(formula = formula1, data = nonDGsample,
                   weights = nonDGsample$base.sales)
      ...
      res1.dt[, n.groups := n.g]
      res1.dt[, lev := quantlab[-1]]
      res1.dt[, sample := "nonDG"]
      
      ## Descriptives
      res1.dt$N_obs <- nrow(nonDGsample)
      res1.dt$N_stores <- uniqueN(nonDGsample, by = c("store_code_uc") )
      res1.dt$N_modules <- length(unique(nonDGsample$product_module_code))
      res1.dt$N_counties <- uniqueN(nonDGsample, by = c("fips_state", "fips_county"))
      res1.dt$N_years <- uniqueN(nonDGsample, by = c("year")) # should be 6 (we lose one because we difference)
      res1.dt$N_county_modules <- uniqueN(nonDGsample, by = c("fips_state", "fips_county",
                                                            "product_module_code"))
      LRdiff_res <- rbind(LRdiff_res, res1.dt, fill = T)
      fwrite(LRdiff_res, iv.output.results.file)
    }
