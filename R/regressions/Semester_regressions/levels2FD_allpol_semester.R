#' Run Non linear estimation using polynomials (parametric) in the FD counterpart
...
## output filepaths ----------------------------------------------
output.results.file <- "Data/allpol_semesterly_levelstodiff.csv"
...
for (n in 2:5) {
  ...
  all_pi[, paste0("D.ln_sales_tax_",n) := paste0("ln_sales_tax_",n) - shift(paste0("ln_sales_tax_",n), n=1, type="lag"),
  ...
  for (Y in c(outcomes)) {
    for (FE in FE_opts) {
      ...
      pred_se <- rep(0,15)
      for (i in 1:15) {
        plc.formula1 <- paste0("D.ln_sales_tax +", paste0(paste0(paste0(2:n,"*",paste0((tax_values[i]),"^",1:(n-1))), "*D.ln_sales_tax_",2:n), collapse = " + "), " = 0")
        # Predictred
      ...
      out.pred.file <- paste0(output.path,"/standard toFD/predict",  Y, "_", n,"_", FE,".csv")
      fwrite(coef.dt, out.pred.file)
      
      # Output file
      graphout <- paste0(output.path,"/standard toFD/", Y, "_", n,"_", FE, ".png")
      # Plot
...
### Run Diff model that matches level twoway FE semester data: hermite polynomials --------------------------------
for (n in 2:5) {
  # First create power
  ...
  for (Y in c(outcomes)) {
    for (FE in FE_opts) {
      ...
      pred_se <- rep(0,15)
      for (i in 1:15) {
        plc.formula1 <- paste0("D.ln_sales_tax +", paste0(paste0(paste0(2:n,"*",(tax_hermite)[1:n-1,i]), "*D.ln_sales_tax_",2:n), collapse = " + "), " = 0")
        # Predictred
        pplc.test1 <- glht(res1, linfct = c(plc.formula1))
        pred_b[i] <- coef(summary(pplc.test1))[[1]]
        pred_se[i] <- sqrt(vcov(summary(pplc.test1)))[[1]]
      ...
      # Create data
      coef.dt <- data.table(tax_values, pred_b, pred_se)
      out.pred.file <- paste0(output.path,"/hermite toFD/predict",  Y, "_", n,"_", FE,".csv")
      fwrite(coef.dt, out.pred.file)
      
      # Output file
      graphout <- paste0(output.path,"/hermite toFD/", Y, "_", n,"_", FE, ".png")
      # Plot
      ggplot(data = coef.dt, mapping = aes(x = tax_values, y = pred_b)) +
        geom_point(size = 2, alpha = .5) +
        geom_line(linetype = "dashed") +
