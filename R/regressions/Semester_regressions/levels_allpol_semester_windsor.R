...
#' We windsorize before estimation
...
## output filepaths ----------------------------------------------
output.results.file <- "Data/allpol_semesterly_levels_windsor.csv"
...
all_pi[, ln_cpricei2 := ifelse(ln_cpricei2 < quantile(ln_cpricei2, probs = 0.05) | ln_cpricei2 > quantile(ln_cpricei2, probs = 0.95),
                               NA, ln_cpricei2)]
all_pi[, ln_quantity3 := ifelse(ln_quantity3 < quantile(ln_quantity3, probs = 0.05) | ln_quantity3 > quantile(ln_quantity3, probs = 0.95),
                               NA, ln_quantity3)]
...
for (n in 2:5) {
  ...
  for (Y in c(outcomes)) {
    for (FE in FE_opts) {
      ...
      res1.dt[, N_obs := nrow(all_pi[!is.na(Y)])]
      ...
      out.pred.file <- paste0(output.path,"/standard w/predict",  Y, "_", n,"_", FE,".csv")
      fwrite(coef.dt, out.pred.file)

      # Output file
      graphout <- paste0(output.path,"/standard w/", Y, "_", n,"_", FE, ".png")
      # Plot
...
for (n in 2:4) {
  ...
  for (Y in c(outcomes)) {
    for (FE in FE_opts) {
      ...
      # Create data
      coef.dt <- data.table(tax_values, pred_b, pred_se)
      out.pred.file <- paste0(output.path,"/hermite prob w/predict",  Y, "_", n,"_", FE,".csv")
      fwrite(coef.dt, out.pred.file)
