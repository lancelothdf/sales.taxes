...
#'
#'Same as LRdiff_diff_semesterly_preferred_cpricei but we plot the cumulative effect as opposed to changes
...
## output filepaths ----------------------------------------------
output.results.file <- "Data/LRdiff_semsterly_results_preferred_cumulative_cpricei.csv"
...
all_pi[, lag.poly0 := L4.D.ln_sales_tax + L3.D.ln_sales_tax + L2.D.ln_sales_tax + L1.D.ln_sales_tax ]
all_pi[, lag.poly1 := 3*L4.D.ln_sales_tax + 2*L3.D.ln_sales_tax + L2.D.ln_sales_tax]
all_pi[, lag.poly2 := 9*L4.D.ln_sales_tax + 4*L3.D.ln_sales_tax + L2.D.ln_sales_tax]
...
for (Y in outcomes) {
  for (FE in FE_opts) {
    ...
    lag.test.form <- "D.ln_sales_tax + 4*lag.poly0"
    for(k in 1:n.poly.lag) {

      tot.lag.n <- 1^k + 2^k + 3^k
...
##!!! NOTE: formula just above assume 2nd degree polynomial for lags
n.poly.lead <- 2
n.poly.lag <- 2
