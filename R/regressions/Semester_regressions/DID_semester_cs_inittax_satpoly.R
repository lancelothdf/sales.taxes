  ...
  for (n in 1:3) {
    ...
    for (n.g in 2:5) {
      ...
      for (FE in FE_opts) {
        ...
        pred_se <- rep(0,15)
        for (i in 1:15) {
          plc.formula1 <- paste0(av.tax.ch, "*(", paste0(paste0(paste0(paste0((tax_values[i]),"^",1:(n))), "*D.ln_sales_tax_init_",1:n), collapse = " + "), ") = 0")
          # Predictred
  ...
  for (n in 1:3) {
    ...
    for (n.g in 2:5) {
      ...
      for (FE in FE_opts) {
        ...
        pred_se <- rep(0,15)
        for (i in 1:15) {
          plc.formula1 <- paste0(av.tax.ch, "*(", paste0(paste0(paste0(paste0((tax_values[i]),"^",1:(n))), "*w.ln_sales_tax_init_",1:n), collapse = " + "), ") = 0")
          # Predictred
          pplc.test1 <- glht(res1, linfct = c(plc.formula1))
          pred_b[i] <- coef(summary(pplc.test1))[[1]]
