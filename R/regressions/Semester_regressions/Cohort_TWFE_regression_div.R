...
## output filepaths ----------------------------------------------
output.results.file <- "Data/Cohort_TWFE_div.csv" ### Main results
boot.results.file <- "Data/Boot_cohort_TWFE_div.csv" ### Bootstrap
...
for(co in c_ids) {
  for (Y in c(outcomes)) {
          ...
          res1 <- lm(formula = formula1, data = all_pi[division_by_module_by_time == co,],
                       weights = all_pi[division_by_module_by_time == co,]$base.sales)
