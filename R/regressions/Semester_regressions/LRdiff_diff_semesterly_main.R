...
#' NOTE: tax rate in a given semester is measured as tax rate in 6th month of that semester
...
## output filepaths ----------------------------------------------
reg.outfile <- "Data/LRdiff_semesterly_results_main.csv"
...
all_pi <- all_pi[, c("fips_state", "fips_county", "year", "semester", "product_module_code", "store_code_uc", "ln_cpricei", "ln_sales_tax", "ln_quantity", "store_by_module", "cal_time", "module_by_time", "module_by_state", "region_by_module_by_time", "division_by_module_by_time", "base.sales", "ln_home_price", "ln_unemp")]
...
lag3.vars <- paste(paste0("L", 3:1, ".D.ln_sales_tax"), collapse = " + ")
lead.lp.restr <- paste(lead.vars, "= 0")
lag.lp.restr <- paste(lag.vars, "+ D.ln_sales_tax = 0")
lag3.lp.restr <- paste(lag3.vars, "+ D.ln_sales_tax = 0")
...
for (Y in c(outcomes, econ.outcomes)) {
  for (FE in FE_opts) {
    ...
    ## sum lags - include only 3 lags (in case it looks like including the last lag or not may influence the results)
    flog.info("Summing lags3...")
    lag3.test <- glht(res1, linfct = lag3.lp.restr)
    ...
    lp.dt <- data.table(
       rn = c("Pre.D.ln_sales_tax", "Post.D.ln_sales_tax", "Post3.D.ln_sales_tax", "All.D.ln_sales_tax"),
       Estimate = c(lead.test.est, lag.test.est, lag3.test.est, total.test.est),
       `Cluster s.e.` = c(lead.test.se, lag.test.se, lag3.test.se, total.test.se),
       `Pr(>|t|)` = c(lead.test.pval, lag.test.pval, lag3.test.pval, total.test.pval),
       outcome = Y,
       controls = FE
    )
...
##### Now control for contemporaneous Econ conditions
print(" Now include contemporaneous econ covariates")

econ.covariates <- c("D.ln_unemp", "D.ln_home_price", "D.ln_unemp + D.ln_home_price")

for (Y in c(outcomes)) {
  for (FE in FE_opts) {
    for(EC in econ.covariates) {

      total_formula <- paste(formula_RHS, " + ", EC, sep = "")
      ...
      res1.dt <- data.table(coef(summary(res1)), keep.rownames=T)
      res1.dt[, outcome := Y]
      res1.dt[, controls := paste(FE, " + ", EC, sep = "")]
      res.table <- rbind(res.table, res1.dt, fill = T)
      res.table <- rbind(res.table, lp.dt, fill = T)
      fwrite(res.table, reg.outfile)
    }
...
form_unemp_lags <- paste0("L", 1:4, ".D.ln_unemp", collapse = "+")
form_unemp_leads <- paste0("F", 1:4, ".D.ln_unemp", collapse = "+")
...
##### Now control for Econ conditions including leads and lags of both unemployment and home price
print(" Now include leads and lags of unemployment + home price")

## generate lags and leads of ln_home_price
for (lag.val in 1:4) {
  ...
  all_pi[, (lead.X) := shift(D.ln_home_price, n=lag.val, type="lead"),
         by = .(store_code_uc, product_module_code)]
}

form_homeprice_lags <- paste0("L", 1:4, ".D.ln_home_price", collapse = "+")
form_homeprice_leads <- paste0("F", 1:4, ".D.ln_home_price", collapse = "+")
form_homeprice_RHS <- paste0("D.ln_home_price + ", form_homeprice_lags, "+", form_homeprice_leads)


for (Y in c(outcomes)) {
  for (FE in FE_opts) {

    total_formula <- paste(formula_RHS, " + ", form_unemp_RHS, "+ ", form_homeprice_RHS, sep = "")
    ...
    lp.dt <- data.table(
      ...
      `Pr(>|t|)` = c(lead.test.pval, lag.test.pval, lag3.test.pval, total.test.pval),
      outcome = Y,
      controls = paste(FE, " + lead-lag unemp & home price", sep = "")
    )
    ...
    res1.dt <- data.table(coef(summary(res1)), keep.rownames=T)
    res1.dt[, outcome := Y]
    res1.dt[, controls := paste(FE, " + lead-lag unemp & home price", sep = "")]
    res.table <- rbind(res.table, res1.dt, fill = T)
    res.table <- rbind(res.table, lp.dt, fill = T)
    fwrite(res.table, reg.outfile)
  }
