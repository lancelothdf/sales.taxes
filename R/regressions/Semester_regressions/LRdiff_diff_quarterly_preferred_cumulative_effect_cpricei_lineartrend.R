...
## output filepaths ----------------------------------------------
output.results.file <- "Data/LRdiff_quarterly_results_preferred_cumulative_cpricei_lineartrend.csv"
...
trend <- c("store_code_uc", "store_by_module")
...
for (Y in outcomes) {
  for (FE in FE_opts) {
    ...
    formula1 <- as.formula(paste0(
      Y, "~", formula_RHS, "| ", FE, " + ", TR, " | 0 | module_by_state"
    ))
