...
theta.output.results.file <- paste0("Data/Demand_gamma_sat_initial_price_semester_boot_r_K_hermite.csv")
...
## Hermite Function Derivative ---------------------------------
d.hermite <-function(x, n, prob = T) {
  if (n < 0) {stop("sequence number n must be non-negative")}
  if (prob) {
    if (n == 0) {return(seq(0,0, length.out = length(x)))}
    if (n >= 1) {return(n*hermite(x, n-1))}
  } else {
    if (n == 0) {return(seq(0,0, length.out = length(x)))}
    if (n > 1) {return(2*n*hermite(x, n-1, prob = F))}
  }
}
...
for (rep in 1:100) {
  ...
  for (n.g in 1:7) {
    ...
    # Calculate integral
    gamma <- ed.price.quantile[ , lapply(.SD, sum), by = .(quantile), .SDcols = paste0("b",1:15)]
    gamma <- gamma[!is.na(quantile),][order(quantile)][, -c("quantile")]
