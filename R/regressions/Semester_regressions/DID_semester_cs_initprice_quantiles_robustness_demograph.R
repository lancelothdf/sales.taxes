#' Sales Taxes Project
#' Robustness Check Heterogeneity: We want to see whether our estimates change by observed characteristics.
#' We have identified characteristics of the stores for 78\% of them. We first check that our BL estimates don't change in this sample.
#' We do this splitting by quantiles as usual.
...
## output filepaths ----------------------------------------------
results.file <- "Data/robust_demog_estimates_initial_price_semester.csv"
theta.results.file <- "Data/Demand_theta_robust_demog_initial_price_semester_boot_r.csv"
...
for (n.g in 1:7) {
  ...
  for (FE in FE_opts) {
    ...
    IV <- LRdiff_res[outcome == "w.ln_quantity3" & n.groups == n.g & controls == FE &
                       sample == "Full",][["Estimate"]]/LRdiff_res[outcome == "w.ln_cpricei2" &
                                                                     n.groups == n.g & controls == FE & sample == "Full",][["Estimate"]]
  ...
  ##### Only Stores with observed characteristics
  het.sample <- all_pi[!is.na(av_hh_income_sales)]
  
  # Create groups of initial values of tax rate
  # We use the full weighted distribution
  het.sample <- het.sample[, quantile := cut(dm.L.ln_cpricei2,
                                           breaks = quantile(dm.L.ln_cpricei2, probs = seq(0, 1, by = 1/n.g), na.rm = T, weight = base.sales),
                                           labels = 1:n.g, right = FALSE)]
  ...
  # Saturate fixed effects
  het.sample[, group_region_by_module_by_time := .GRP, by = .(region_by_module_by_time, quantile)]
  het.sample[, group_division_by_module_by_time := .GRP, by = .(division_by_module_by_time, quantile)]
  ...
  for (FE in FE_opts) {
    for (Y in outcomes) {
      ...
      if (n.g == 1) { formula1 <- as.formula(paste0(Y, " ~ w.ln_sales_tax | ", FE)) }
      res1 <- felm(formula = formula1, data = het.sample,
                   weights = het.sample$base.sales)
      ...
      res1.dt[, n.groups := n.g]
      res1.dt[, lev := quantlab[-1]]
      res1.dt[, sample := "Het.Sample"]
      
      ## Descriptives
      res1.dt$N_obs <- nrow(het.sample)
      res1.dt$N_stores <- uniqueN(het.sample, by = c("store_code_uc") )
      res1.dt$N_modules <- length(unique(het.sample$product_module_code))
      res1.dt$N_counties <- uniqueN(het.sample, by = c("fips_state", "fips_county"))
      res1.dt$N_years <- uniqueN(het.sample, by = c("year")) # should be 6 (we lose one because we difference)
      res1.dt$N_county_modules <- uniqueN(het.sample, by = c("fips_state", "fips_county",
                                                            "product_module_code"))
      LRdiff_res <- rbind(LRdiff_res, res1.dt, fill = T)
      fwrite(LRdiff_res, results.file)
    }
    ...
    if (n.g > 1) {
      # Get the empirical distribution of prices by quantile
      het.sample[, base.sales.q := base.sales/sum(base.sales), by = .(quantile)]
      het.sample[, p_group := floor((dm.ln_cpricei2 - min(dm.ln_cpricei2, na.rm = T))/((max(dm.ln_cpricei2, na.rm = T)-min(dm.ln_cpricei2, na.rm = T))/500)), by = .(quantile)]
      het.sample[, p_ll := p_group*((max(dm.ln_cpricei2, na.rm = T)-min(dm.ln_cpricei2, na.rm = T))/500), by = .(quantile)]
      het.sample[, p_ll := p_ll + min(dm.ln_cpricei2, na.rm = T), by = .(quantile)]
      het.sample[, p_ul := p_ll + ((max(dm.ln_cpricei2, na.rm = T)-min(dm.ln_cpricei2, na.rm = T))/500), by = .(quantile)]
      
      ed.price.quantile <- het.sample[, .(w1 = (sum(base.sales.q))), by = .(p_ul, p_ll, quantile)]
      ed.price.quantile[, p_m := (p_ul+p_ll)/2]
      ...
      estimated.target[, n.groups := n.g]
      estimated.target[, controls := FE]
      estimated.target[, sample := "Het.Sample"]
      target_res <- rbind(target_res, estimated.target)
      fwrite(target_res, theta.results.file)
    }
  }
