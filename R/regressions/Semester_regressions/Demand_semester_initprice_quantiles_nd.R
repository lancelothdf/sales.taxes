...
## output filepaths ----------------------------------------------
iv.output.results.file <- "Data/Demand_iv_sat_initial_price_semester_nd.csv"
theta.output.results.file <- "Data/Demand_theta_sat_initial_price_semester_nd.csv"
...
for (n.g in 2:7) {
  ...
  for (FE in FE_opts) {
    ...
    all_pi[, p_group := floor((ln_cpricei2 - min(ln_cpricei2, na.rm = T))/((max(ln_cpricei2, na.rm = T)-min(ln_cpricei2, na.rm = T))/100)), by = .(quantile)]
    all_pi[, p_ll := p_group*((max(ln_cpricei2, na.rm = T)-min(ln_cpricei2, na.rm = T))/100), by = .(quantile)]
    all_pi[, p_ll := p_ll + min(ln_cpricei2, na.rm = T), by = .(quantile)]
    all_pi[, p_ul := p_ll + ((max(ln_cpricei2, na.rm = T)-min(ln_cpricei2, na.rm = T))/100), by = .(quantile)]
    ...
    mean.p <- all_pi[, mean(ln_cpricei2, weights = base.sales)]
