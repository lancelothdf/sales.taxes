...
## output filepaths ----------------------------------------------
output.results.file <- "Data/allpol_semesterly_diff_windsor.csv"
...
for (n in 2:3) {
  ...
  for (Y in c(outcomes)) {
    for (FE in FE_opts) {
      ...
      out.pred.file <- paste0(output.path,"/D standard w/predict", Y, "_", n,"_", FE,".csv")
      fwrite(coef.dt, out.pred.file)
      
      # Output file
      graphout <- paste0(output.path,"/D standard w/", Y, "_", n,"_", FE, ".png")
      # Plot
...
for (n in 2:4) {
  ...
  for (Y in c(outcomes)) {
    for (FE in FE_opts) {
      ...
      # Create data
      coef.dt <- data.table(tax_values, pred_b, pred_se)
      out.pred.file <- paste0(output.path,"/D hermite prob w/predict", Y, "_", n,"_", FE,".csv")
      fwrite(coef.dt, out.pred.file)
      
      # Output file
      graphout <- paste0(output.path,"/D hermite prob w/", Y, "_", n,"_", FE, ".png")
      # Plot
      ggplot(data = coef.dt, mapping = aes(x = tax_values, y = pred_b)) +
        geom_point(size = 2, alpha = .5) +
        geom_line(linetype = "dashed") +
