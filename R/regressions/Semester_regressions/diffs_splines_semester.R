...
## output filepaths ----------------------------------------------
output.results.file <- "Data/splines_semesterly_diffs.csv"
output.knots.file <- "Data/splines_semesterly_diffs_knots.csv"
...
av.tax.ch <- 1 # We want the effect at \Detal \tau = 1
...
for (k in 3:10) {
  ...
  for (n in 1:2) {
    ...
    if (n > 1) {
      ...
      # Add to formula
      RHS <- paste(RHS, paste0(paste0("D.ln_sales_tax_init_",2:n), collapse = " + "), sep = " + ")
    }
    # Second create truncated function
    d <-1
    for (ep in knots) {
      all_pi[, paste0("D.ln_sales_tax_init_k",d) := (L.ln_sales_tax > ep)*D.ln_sales_tax*(L.ln_sales_tax - ep)^(n)]
      d <- d+1
    }
    
    # Add trunctaed terms to formula
    RHS <- paste(RHS, paste0(paste0("D.ln_sales_tax_init_k",1:k), collapse = " + "), sep = " + ")
    ...
    for (Y in c(outcomes)) {
      for (FE in FE_opts) {
        ...
        for (i in 1:15) {
          ...
          if (n >1) {
            plc.formula1pk <- paste0((tax_values[i]), "*D.ln_sales_tax_init +", 
                                     paste0(paste0(paste0((tax_values[i]),"^",2:n), "*D.ln_sales_tax_init_",2:n), 
                                            collapse = " + "))
          } else {
          ...
          for (ep in knots) {
            if ((tax_values[i]) > ep) {
              plc.formula1pk <- paste0(plc.formula1pk, "+", paste0(((tax_values[i]) - ep)^(n), "*","D.ln_sales_tax_init_k",d))
            }
            d <- d+1
          }
          plc.formula1 <- paste0(av.tax.ch, "*(D.ln_sales_tax + ", plc.formula1pk, ") = 0")
