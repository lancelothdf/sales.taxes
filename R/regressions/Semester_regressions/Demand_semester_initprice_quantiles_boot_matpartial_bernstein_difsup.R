...
#' This version: we play around with what the support is 
...
## output filepaths ----------------------------------------------
pq.output.results.file <- "Data/Demand_pq_sat_initial_price_semester_difsup.csv"
output.path <- "Data/Demand_gamma_sat_initial_price_semester_difsup_K"
...
# Before cutting, get also the %2 and 5%
pct2 <- quantile(all_pi$dm.ln_cpricei2, probs = 0.02, na.rm = T, weight=base.sales)
pct3 <- quantile(all_pi$dm.ln_cpricei2, probs = 0.03, na.rm = T, weight=base.sales)
pct4 <- quantile(all_pi$dm.ln_cpricei2, probs = 0.04, na.rm = T, weight=base.sales)
pct5 <- quantile(all_pi$dm.ln_cpricei2, probs = 0.05, na.rm = T, weight=base.sales)
pct95 <- quantile(all_pi$dm.ln_cpricei2, probs = 0.95, na.rm = T, weight=base.sales)
pct96 <- quantile(all_pi$dm.ln_cpricei2, probs = 0.96, na.rm = T, weight=base.sales)
pct97 <- quantile(all_pi$dm.ln_cpricei2, probs = 0.97, na.rm = T, weight=base.sales)
pct98 <- quantile(all_pi$dm.ln_cpricei2, probs = 0.98, na.rm = T, weight=base.sales)

# Now cut 
all_pi <- all_pi[(dm.ln_cpricei2 > pct1 & dm.ln_cpricei2 < pct99),]

# Define others to re-escale
p.down <- c(pct1, pct2, pct3, pct4, pct5)
p.up <- c(pct99, pct98, pct97, pct96, pct95)


pq_res <- data.table(NULL)

## To estimate the intercept (this is constant)
mean.q <- all_pi[, mean(ln_quantity3, weights = base.sales, na.rm = T)]
for (cut in 1:length(p.up)) {
  ...
  # extract
  estimated.pq <- data.table(mean.q, mean.p, min.p, max.p, cut)
