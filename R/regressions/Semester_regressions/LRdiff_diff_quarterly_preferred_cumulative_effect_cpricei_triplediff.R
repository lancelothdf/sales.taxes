...
## output filepaths ----------------------------------------------
output.results.file <- "Data/LRdiff_quarterly_results_preferred_cumulative_cpricei_triplediff.csv"
...
all_pi[, store_by_time := .GRP, by = .(store_code_uc, cal_time)]
...
all_pi <- all_pi[, c("fips_state", "fips_county", "year", "quarter", "store_code_uc", "product_module_code", "ln_cpricei", "ln_sales_tax", "ln_quantity", "base.sales", "store_by_module", "store_by_time", "cal_time", "module_by_time", "module_by_state", "region_by_module_by_time", "division_by_module_by_time", "ln_home_price", "ln_unemp")]
...
for (Y in outcomes) {
  for (FE in FE_opts) {
    ...
    formula1 <- as.formula(paste0(
      Y, "~", formula_RHS, "| store_by_time + ", FE, " | 0 | module_by_state"
    ))
    ...
    res1.dt[, outcome := Y]
    res1.dt[, controls := paste("store_by_time + ", FE, sep = "")]
    res1.dt[, econ := "none"]
    ...
    lp.dt <- data.table(
      ...
      outcome = Y,
      controls = paste("store_by_time + ", FE, sep = ""),
      econ = "none",
      parametric = "Yes",
