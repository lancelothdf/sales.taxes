...
## output filepaths ----------------------------------------------
reg.outfile <- "Data/semesterly_pi_output.csv"
...
for (lag.val in 1:4) {
  ...
  lag.X <- paste0("L", lag.val, ".ln_sales_tax_Q2")
  all_pi[, (lag.X) := shift(ln_sales_tax_Q2, n=lag.val, type="lag"),
         by = .(store_code_uc, product_module_code)]

  lead.X <- paste0("F", lag.val, ".ln_sales_tax_Q2")
  all_pi[, (lead.X) := shift(ln_sales_tax_Q2, n=lag.val, type="lead"),
         by = .(store_code_uc, product_module_code)]
}
...
formula_lags <- paste0("L", 1:4, ".ln_sales_tax", collapse = "+")
formula_leads <- paste0("F", 1:4, ".ln_sales_tax", collapse = "+")
formula_RHS <- paste0("ln_sales_tax + ", formula_lags, "+", formula_leads)

formula_lags_Q2 <- paste0("L", 1:4, ".ln_sales_tax_Q2", collapse = "+")
formula_leads_Q2 <- paste0("F", 1:4, ".ln_sales_tax_Q2", collapse = "+")
formula_RHS_Q2 <- paste0("ln_sales_tax_Q2 + ", formula_lags_Q2, "+", formula_leads_Q2)

outcomes <- c("ln_cpricei", "ln_quantity")
outcomes.Q2 <- c("ln_cpricei_Q2", "ln_quantity_Q2")
FE_opts <- c("cal_time + store_by_module",
             "module_by_time + store_by_module",
             "region_by_module_by_time + store_by_module",
             "division_by_module_by_time + store_by_module")

## for linear hypothesis tests
lead.vars <- paste(paste0("F", 4:1, ".ln_sales_tax"), collapse = " + ")
lag.vars <- paste(paste0("L", 4:1, ".ln_sales_tax"), collapse = " + ")
lead.vars.Q2 <- paste(paste0("F", 4:1, ".ln_sales_tax_Q2"), collapse = " + ")
lag.vars.Q2 <- paste(paste0("L", 4:1, ".ln_sales_tax_Q2"), collapse = " + ")
lead.lp.restr <- paste(lead.vars, "= 0")
lead.lp.restr.Q2 <- paste(lead.vars.Q2, "= 0")
lag.lp.restr <- paste(lag.vars, "+ ln_sales_tax = 0")
lag.lp.restr.Q2 <- paste(lag.vars.Q2, "+ ln_sales_tax_Q2 = 0")
total.lp.restr <- paste(lag.vars, "+", lead.vars, "+ ln_sales_tax = 0")
total.lp.restr.Q2 <- paste(lag.vars.Q2, "+", lead.vars.Q2, "+ ln_sales_tax_Q2 = 0")
...
for (Y in c(outcomes, outcomes.Q2)) {
  for (FE in FE_opts) {
    ...
    ## linear hypothesis results
    lp.dt <- data.table(
      rn = c("Pre.ln_sales_tax_Q2", "Post.ln_sales_tax_Q2", "All.ln_sales_tax_Q2"),
      Estimate = c(lead.test.est, lag.test.est, total.test.est),
      `Cluster s.e.` = c(lead.test.se, lag.test.se, total.test.se),
      `Pr(>|t|)` = c(lead.test.pval, lag.test.pval, total.test.pval),
      outcome = Y,
