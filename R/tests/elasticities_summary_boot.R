...
## Outputs ----------------------------------------------
output.table <- "Data/summary_elasticity_states_boot.csv"
...
bounds.full <- fread(bounds.data)

## Keep all bounds for d <=3 and K <= 10
bounds.full <- bounds.full[ D <= 3 & K %in% seq(2,10,1)]
...
for (rep in 0:76) {
  ...
  if (rep >0) {
    ...
    elasticities <- merge(sampled.ids, all_pi, by = c("module_by_state") , allow.cartesian = T, all.x = T)
  ...
  bounds <- bounds.full[iter == rep][, -c("iter")]
  ...
  elasticities[, weight := "consumer price"]
