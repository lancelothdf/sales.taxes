...
# Non-Marginal Case
# Run in serial for a couple of states to see code is working properly in actual data
# Scale to parallel by steps
...
## Output files
out.file.nonmarginal <- "Data/nonmarginal_extrapoaltion_state_ser.csv"
      ...
      for (D in 2) { #unique(gamma$n.groups)
        ...
        for (theta in thetas.test) {
          ...
          for (state in states.test) {
            ...
            welfare.st <- data.table(down, up, state, sc, D , K, sigma = sig, theta, s1, s2, it1, it2, ConsChckDown, ConsChckUp)
