...
output.results.filepath <- "Data/pi_ei_regression_res_prodmatch_combined_FHS.csv"
output.residuals.cpricei.filepath <- "Data/pi_ei_regression_prodmatch_residuals_cpricei_combined_FHS.csv"
output.residuals.tax.filepath <-"Data/pi_ei_regression_prodmatch_residuals_tax_combined_FHS.csv"
output.xx.filepath <- "Data/pi_ei_regression_prodmatch_xx_combined_FHS.csv"
...
all_pi <- merge(unemp_dt, all_pi, by = c("fips_county", "fips_state", "quarter", "year"))
...
for (yr in 2009:2013) {
  for (qtr in 1:4) {
    ...
    ## need new_cols_used minus one of the leads, plus unemp_rate, adding the
    ## newly subtracted lead as an instrument for the unemp_rate
    cXp_formula <- as.formula(paste0("cpricei ~ ", felm_formula_input,
                                       " | county_ID + tt_event | (unemp_rate ~ cattlead1) | county_ID"))
    ...
    setDT(resid)
    resid.cpricei <- resid[, list(cattlead4 = sum(weights*cattlead4*residuals), cattlead3 = sum(weights*cattlead3*residuals), catt0 = sum(weights*catt0*residuals), catt1 = sum(weights*catt1*residuals), catt2 = sum(weights*catt2*residuals), catt3 = sum(weights*catt3*residuals), catt4 = sum(weights*catt4*residuals), weights = sum(weights)), by = "county_ID"]
...
###OUTPUT
output.cov.cpricei <- "Data/pi_ei_regression_prodmatch_cpricei_varcov_matrix_combined_FHS.csv"
output.skeleton <- "Data/pi_ei_regression_prodmatch_varcov_matrix_skeleton_combined_FHS.csv"
...
#for(cty in list.counties[1:3])
  ...
  c.cpricei <- c.cpricei %>% gather(key = "lead", value = "parameter", "cattlead4", "cattlead3", "catt0", "catt1", "catt2", "catt3", "catt4")
...
write.table(residual.mat, "Data/large_vcov_matrices/Mat_residuals_prodmatch_ei_pi_combined_FHS.csv")
...
output.estimates.stderr.filepath <- "Data/Passthrough_estimates_stderr_ei_combined_FHS.csv"
...
estprice.pass <- estimates[outcome == 'cpricei' & rn %in% c("catt-4", "catt-3", "catt0", "catt1", "catt2", "catt3", "catt4"), "estimates"]
