...
data_path <- "output/server/pi_data/ever_increase/unemprate_all_cohorts_ei_pooled.csv"
outfile_figpath <- "reports/figs/unemp_all_cohorts_ei_pooled_extended.png"
outfile_figpath2 <- "reports/figs/unemp_all_cohorts_ei_pooled_extended_normalized.png"

dt <- read.csv(data_path)

ggplot(dt, mapping = aes(x = tt_event, y = mean_rate, color = tr_group)) +
  geom_line(size = .7) +
  geom_point(size = .8) +
...
dt.norm <- dt %>%
  group_by(tr_group) %>%
  mutate(mean_rate = mean_rate - mean_rate[tt_event == -6])

ggplot(dt.norm, mapping = aes(x = tt_event, y = mean_rate, color = tr_group)) +
  geom_line(size = .7) +
  geom_point(size = .8) +
  geom_vline(xintercept = 0, color = "red", linetype = "22", alpha = .5) +
  theme_bw(base_size = 14) +
