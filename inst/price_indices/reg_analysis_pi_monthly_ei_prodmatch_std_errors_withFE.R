...
output.results.filepath <- "Data/pi_ei_monthly_regression_res_prodmatch_withFE.csv"
output.residuals.cpricei.filepath <- "Data/pi_ei_monthly_regression_prodmatch_residuals_cpricei_withFE.csv"
output.residuals.tax.filepath <-"Data/pi_ei_monthly_regression_prodmatch_residuals_tax_withFE.csv"
output.xx.filepath <- "Data/pi_ei_monthly_regression_prodmatch_xx_withFE.csv"
...
setDT(geo_dt)
geo_dt <- geo_dt[, .(fips_state, division)]
...
for (yr in 2009:2013) {
  for (mon in 1:12) {
    ...
    ## merge on Census region
    ss_pi <- merge(ss_pi, geo_dt, by = "fips_state")
    ss_pi[, division_by_time := .GRP, by = .(division, tt_event)]
    flog.info("Merged on Census division.")
    ...
    cXp_formula <- as.formula(paste0("cpricei ~ ", felm_formula_input,
                                       " | county_ID + division_by_time | 0 | county_ID"))
    ...
    get.res2 <- function(var) { return(get.res(var, "county_ID + division_by_time", "weights", ss_pi)) } #Make get.res a function of 1 variable only to iterate over
...
###OUTPUT
output.cov.cpricei <- "Data/pi_ei_monthly_regression_prodmatch_cpricei_varcov_matrix_withFE.csv"
output.skeleton <- "Data/pi_ei_monthly_regression_prodmatch_varcov_matrix_skeleton_withFE.csv"
...
write.table(residual.mat, "Data/large_vcov_matrices/Mat_residuals_prodmatch_ei_monthly_pi_withFE.csv")
...
output.estimates.stderr.filepath <- "Data/Passthrough_estimates_stderr_ei_monthly_withFE.csv"
